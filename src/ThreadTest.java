...
public class ThreadTest {
    ...
    public static void main(String[] args) {
        Thread waittingBlocked = new Thread(new WaitingBlocked(), "waittingBlocked");
        Thread timeWaittingBlocked = new Thread(new TimeWaitBlocked(), "timeWaittingBlocked");
        ...
        Thread joinBlocked = new Thread(new JoinBlocked(mainTherad), "joinBlocked");
...
/**
 * 等待阻塞，一直处于等待状态，知道有其他线程将其唤醒 * * @author xiaotao *
 */
...
/**
 * 在五十秒内处于等待状态，过了五十秒之后会被唤醒，进入runnable状态 * * @author xiaotao *
 */
    ...
    public void run() {
        try {
            synchronized (TimeWaitBlocked.class) {
                ...
                System.out.println("l have wait about 50 s,so i don't want to wait");
            }
        } catch (InterruptedException e) {
            e.printStackTrace();
        }
...
/**
 * 该实例是一个抢锁的阻塞实例，两个线程争抢一个锁，只有一个线程抢到锁，另外一个线程抢不到锁，处于阻塞状态 * * @author xiaotao *
 */
...
/**
 * 处于sleep阻塞状态，睡了100秒之后处于runnable状态，之后争抢cpu。 * * @author xiaotao *
 */
